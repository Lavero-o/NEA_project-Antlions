class_name EntitySelector extends Resource

signal entities_selected(entities: Array[Entity])

const SELECTION_AREA_SCENE: PackedScene = preload("uid://coxs4q4gumdpt")
const SELECTION_RECT_SCENE: PackedScene = preload("uid://c7p1bp5526eom")

var selection_area_scene: Area2D
var selection_rect_scene: ColorRect
...
func _ready() -> void:
	canvas = get_local_scene().find_child(Globals.canvas_name)
	if not canvas:
		canvas = CanvasLayer.new()
		get_local_scene().add_child(canvas)
