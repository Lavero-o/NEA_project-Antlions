...
func handle_zoom_input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed('zoom_in') : player.zoom_in(zoom_amount)
	if Input.is_action_just_pressed('zoom_out') : player.zoom_out(zoom_amount)
...
func handle_pan(event: InputEvent) -> void:
	...
	if not (event is InputEventMouseMotion or panning) : return
